...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyRiwayatFilter() {
        ...
        // Use direct child selector to avoid matching nested selects/buttons with data-book-id
    ...
    applyRiwayatFilter(); // init on load
    ...
    async function searchGoogleBooks(query) {
        ...
        const url = `${GOOGLE_BOOKS_API}?q=${encodeURIComponent(query)}&maxResults=8&printType=books&orderBy=relevance${keyParam}`;
        const res = await fetch(url, { headers: { Accept: 'application/json' } });
        if (!res.ok) throw new Error('Gagal menghubungi Google Books');
    ...
    addBtn?.addEventListener('click', async () => {
        if (!selectedBook || addBtn.disabled) return;
        ...
        try {
            ...
            if (!resp.ok) throw new Error(result.message || 'Gagal menyimpan');
            location.reload();
        } catch (err) {
