    ...
    <div class="min-h-screen pt-16">
        <div class="flex items-start gap-6 max-w-300 mx-auto px-4 py-6 max-md:pb-24">
            ...
            <article class="bg-white border-[1.5px] border-[#444] rounded-3xl p-5 mb-4" data-post-id="{{ $post['id'] }}">
                    ...
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="flex flex-col min-w-0">
                            <div class="flex flex-wrap items-center gap-1.5">
                                <a href="{{ url('/u/' . ltrim($post['handle'], '@')) }}" class="font-bold text-[1.05rem] text-[#222] hover:underline leading-tight truncate block">{{ $post['name'] }}</a>
                                @if($post['klub'])
                                    <span class="text-[0.75rem] text-gray-500 font-medium whitespace-nowrap bg-gray-100 px-2 py-0.5 rounded-full border border-gray-200">di <span class="font-semibold text-[#222]">{{ $post['klub'] }}</span></span>
                                @endif
                            </div>
                            <a href="{{ url('/u/' . ltrim($post['handle'], '@')) }}" class="text-[0.9rem] text-gray-500 hover:underline mt-0.5 block truncate">{{ $post['handle'] }}</a>
                        </div>
                    </div>
                ...
                <!-- Text Content -->
                <p class="text-[#222] text-[1.1rem] leading-relaxed mb-4 whitespace-pre-wrap">{{ $post['body'] }}</p>

                <!-- Book Tag -->
                @if($post['book'])
                <div class="mb-4">
                    <div class="inline-flex items-start px-3.5 py-2 bg-[#FFDDAF] border-[1.5px] border-[#444] rounded-xl text-[0.85rem] font-bold text-[#444] max-w-full">
                        <span class="break-words leading-tight">{{ $post['book'] }}</span>
                    </div>
                </div>
                @endif
                ...
                <!-- Date & Location -->
                <div class="flex flex-wrap items-center gap-1.5 text-[0.9rem] text-gray-500 py-3 border-t border-gray-100 mb-2">
                    ...
                    <form data-comment-form class="flex gap-3 items-start">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-2 mt-2">
                                ...
                                <button type="submit" data-comment-submit class="w-9 h-9 flex items-center justify-center bg-[#FFDDAF] border-[1.5px] border-[#444] rounded-full hover:bg-[#ffcf90] transition-colors shadow-[2px_2px_0_0_rgba(68,68,68,1)] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shrink-0" aria-label="Kirim">
