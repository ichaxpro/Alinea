        ...
        <div class="flex items-start gap-6 max-w-300 mx-auto px-4 py-6 max-md:pb-24">
            ...
            <main class="bg-white border-[1.5px] border-[#444] rounded-2xl overflow-hidden flex flex-col flex-1">
                ...
                <div class="divide-y-[1.5px] divide-gray-200">
                    ...
                    @endphp
                    <div class="p-4 hover:bg-gray-50 transition-colors flex gap-4 items-start {{ (isset($notif['type']) && in_array($notif['type'], ['user_warning', 'post_hidden', 'klub_hidden', 'review_hidden', 'content_hidden'])) ? 'bg-red-50' : ($notification->read_at ? '' : 'bg-blue-50/30') }}">
                        ...
                        <div class="flex-shrink-0 pt-0.5">
                            ...
                            @elseif(isset($notif['type']) && in_array($notif['type'], ['user_warning', 'post_hidden', 'klub_hidden', 'review_hidden', 'content_hidden']))
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-1.5">
                                {{-- Avatar User / Sistem --}}
                                @if(isset($notif['type']) && in_array($notif['type'], ['user_warning', 'post_hidden', 'klub_hidden', 'review_hidden', 'content_hidden']))
                                ...
                                @endif
                                {{-- Info Pengirim --}}
                                <span class="font-bold text-sm {{ (isset($notif['type']) && in_array($notif['type'], ['user_warning', 'post_hidden', 'klub_hidden', 'review_hidden', 'content_hidden'])) ? 'text-red-700' : 'text-[#444]' }}">{{ (isset($notif['type']) && in_array($notif['type'], ['user_warning', 'post_hidden', 'klub_hidden', 'review_hidden', 'content_hidden'])) ? 'Peringatan Admin' : $finalName }}</span>
                                <span class="text-xs text-gray-400 ml-auto flex-shrink-0">{{ $time }}</span>
                            </div>
                            
                            {{-- Teks Detail Notifikasi --}}
                            <p class="text-xs leading-relaxed pl-9 break-words {{ (isset($notif['type']) && in_array($notif['type'], ['user_warning', 'post_hidden', 'klub_hidden', 'review_hidden', 'content_hidden'])) ? 'text-red-600 font-medium' : 'text-gray-600' }}">
                                @if(isset($notif['type']) && in_array($notif['type'], ['user_warning', 'post_hidden', 'klub_hidden', 'review_hidden', 'content_hidden']))
                                    {{ $notif['message'] }}
                                @else
