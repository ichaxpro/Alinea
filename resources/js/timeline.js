        ...
        return '<div class="flex gap-3 rounded-xl bg-gray-50 p-3 border border-gray-100">' +
            ...
            '<div class="min-w-0 flex-1">' +
                '<div class="flex flex-wrap items-center gap-2 text-xs text-gray-400 mb-1">' +
                    ...
                    '<span class="whitespace-nowrap" title="' + escapeHtml(comment.time || '') + '">' + escapeHtml(comment.absolute_time || 'Baru saja') + '</span>' +
                '</div>' +
...
document.addEventListener('click', (e) => {
    ...
    document.querySelectorAll('[data-post-menu-dropdown], [data-share-dropdown]').forEach(d => {
        if (d !== postMenuDropdown && d !== shareDropdown) {
            d.classList.add('hidden');
        }
    });
    
    if (postMenuTrigger) {
        const menu = postMenuTrigger.nextElementSibling;
        if (menu && menu.hasAttribute('data-post-menu-dropdown')) {
            menu.classList.toggle('hidden');
        }
    }
    
    if (shareTrigger) {
        const menu = shareTrigger.nextElementSibling;
        if (menu && menu.hasAttribute('data-share-dropdown')) {
            menu.classList.toggle('hidden');
        }
    }
});
...
document.addEventListener('click', async (e) => {
    ...
    if (deleteBtn) {
        ...
        // Hide the kebab menu when modal opens
        ...
        try {
            ...
            if (typeof window.showToast === 'function') {
                window.showToast('Unggahan berhasil dihapus.');
            } else {
                console.log('Unggahan berhasil dihapus.');
            }
        } catch (err) {
            deleteBtn.disabled = false;
            if (typeof window.showToast === 'function') {
                window.showToast('Gagal menghapus unggahan.');
            } else {
                alert('Gagal menghapus unggahan.');
            }
        }
    }
});

// ── Share Features (Salin Tautan & Bagikan via Chat) ──
...
document.addEventListener('click', async (e) => {
    ...
    if (copyBtn) {
        ...
        try {
            ...
            if (typeof window.showToast === 'function') {
                window.showToast('Tautan berhasil disalin ke papan klip.');
            } else {
                console.log('Tautan disalin: ' + url);
            }
        } catch (err) {
                ...
                usersToRender.forEach(user => {
                    ...
                    sendBtn.addEventListener('click', async () => {
                        ...
                        try {
                            ...
                            if (typeof window.showToast === 'function') {
                                window.showToast(`Dibagikan ke ${user.name}`);
                            }
                        } catch (err) {
                            console.error(err);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function openReportModal(postId) {
        ...
        void reportModal.offsetWidth; // trigger reflow
    ...
    reportForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (typeof window.showToast === 'function') {
                window.showToast(data.message || 'Laporan berhasil dikirim');
            } else {
    ...
    // Delegated click for Report and Unfollow buttons
    document.addEventListener('click', async (e) => {
        ...
        if (unfollowBtn) {
            ...
            try {
                ...
                if (typeof window.showToast === 'function') {
                    window.showToast('Berhasil berhenti mengikuti.');
                }

                // Hide posts from this user from the timeline
