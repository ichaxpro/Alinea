...
export function renderCommentPanel(post, currentUserAvatar, currentUserName) {
    ...
    return `
        <div data-comments-panel class="hidden mt-4 pt-4 border-t border-gray-100" data-comments-loaded="false" data-comments-limit="5" data-comments-url="${commentsUrl}" data-comments-store-url="${storeUrl}">
...
export function createPostElement(post, currentUserAvatar, currentUserName) {
    ...
    article.innerHTML = `
        <div class="flex items-center gap-3 mb-3 justify-between">
            <a href="${post.profile_url || '#'}" class="w-11 h-11 rounded-full border-2 border-[#444] flex-shrink-0 overflow-hidden bg-gradient-to-br from-[#FFDDAF] to-[#C7E7FF] flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity">
                ${renderAvatarHtml(post)}
            </a>
            <div class="flex-1">
        ...
        <p class="text-sm max-sm:text-xs text-gray-600 leading-relaxed mb-4 whitespace-pre-wrap">${escapeHtml(post.body || '')}</p>
        ${mediaHtml}
        ...
        ${renderCommentPanel(post, currentUserAvatar, currentUserName)}
    `;
...
export function bindPostActions(scope, csrfToken) {
    ...
    scope.querySelectorAll('[data-comment-delete]').forEach(btn => {
        if (btn.dataset.bound === 'true') return;
        btn.dataset.bound = 'true';
        btn.addEventListener('click', async () => {
            ...
            const commentId = btn.dataset.commentId;
            const commentItem = btn.closest('.flex.gap-3');
            const article = btn.closest('article[data-post-id]');
            
            try {
                const res = await fetch(`/timeline_home/comments/${commentId}`, {
                    method: 'DELETE',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': csrfToken,
                ...
                import('./toast.js').then(m => m.showToast('Komentar berhasil dihapus.'));
            } catch (e) {
                import('./toast.js').then(m => m.showToast('Gagal menghapus komentar.'));
            }
        });
    });
}
